...
def get_dissimilarity_scores(patches: list, predictions_matrix: np.ndarray,
                             rotations_shuffled: bool = True) -> np.ndarray:
    """
...
def compatibility_score(dissimilarity_scores: np.ndarray, best_neighbors: np.ndarray, patch_index: int, r: int,
                        rotations_shuffled: bool = True) -> np.ndarray:
    """
    TODO
    :param dissimilarity_scores:
...
def get_compatibility_scores(dissimilarity_scores: np.ndarray, best_neighbors: np.ndarray,
                             rotations_shuffled: bool = True) -> np.ndarray:
    """
...
def pick_first_piece(buddy_matrix: np.ndarray, compatibility_scores: np.ndarray, best_neighbors: np.ndarray,
                     rotations_shuffled: bool = True) -> int:
    """
    ...
    for i in range(n):
        ...
        if passes:
            candidates.append(i)
    buddy_counts = [sum([buddy_matrix[buddy_index, r] is not None for r in range(buddy_matrix.shape[1])]) for
                    buddy_index in range(buddy_matrix.shape[0])]
...
def addBuddies(placedPiece, checkMutuality, checkCycles):
    ...
    if (numPlacedNeighborsOfNext == 1 and
            (bestBuddies[next][dir[0]] != -1
             ...
             and bestBuddies[bestBuddies[bestBuddies[next][dir[0]]][dir[1]]][dir[2]] == placedPiece)):
        cycleBonus += 0.25
    ...
    if (numPlacedNeighborsOfNext == 1 and
            (bestBuddies[next][dir[2]] != -1
             ...
             and bestBuddies[bestBuddies[bestBuddies[next][dir[2]]][dir[1]]][dir[0]] == placedPiece)):
        cycleBonus += 0.25
    ...
    if (construction_matrix[nextRow + originIndex][nextCol + originIndex] == EXPANSION_SPACE):  # if spot is available
        ...
        if (checkCycles):
            placementScore += cycleBonus
            placementScore += bestNeighborBonus * 0.2

        elif not checkMutuality:  # *does* get a bonus for having more neighbors
...
def jigsaw_pt(patches: list, rotations_shuffled: bool = True):
    ...
    solved_puzzle = solve_puzzle(patches, first_piece, dissimilarity_scores, compatibility_scores, buddy_matrix,
                                 rotations_shuffled)
    return None
