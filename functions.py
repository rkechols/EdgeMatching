...
def boring_score(combo_patch: np.ndarray) -> float:
    ...
    for seam_index in [middle_seam_index1 - 1, middle_seam_index1, middle_seam_index2, middle_seam_index2 + 1]:
        for vertical_shift in range(1, 5):
            diff_array = abs(
                combo_patch[:(-1 * vertical_shift), seam_index, :] - combo_patch[vertical_shift:, seam_index, :])
            diffs += list(diff_array.flatten())
    # average list at the end
...
def kl_score(combo_patch: np.ndarray) -> float:
    ...
    distributions_left = [data_to_probability_distribution(left[:, :, c], -0.5, 255.5, 64) for c in
                          range(left.shape[2])]
    distributions_right = [data_to_probability_distribution(right[:, :, c], -0.5, 255.5, 64) for c in
                           range(left.shape[2])]
    kl_values = [kl_divergence_symmetric(d_left, d_right) for d_left, d_right in
                 zip(distributions_left, distributions_right)]
...
def assemble_patches(patches: list, num_cols: int) -> Union[None, np.ndarray]:
    ...
    for i, patch in enumerate(patches):
        ...
        assembled[start_pixel_row:(start_pixel_row + patch_size), start_pixel_col:(start_pixel_col + patch_size),
        :] = patch
    return assembled
...
def assemble_image(patches: list, construction_matrix: np.ndarray) -> Union[None, np.ndarray]:
    ...
    for i in range(construction_matrix.shape[0]):
        for j in range(construction_matrix.shape[1]):
            ...
            # put it in its place
            assembled[(i * patch_size):(patch_size * (i + 1)), (j * patch_size):(patch_size * (j + 1)),
            :] = rotated_patch
    return assembled
